...
import type { CaptureInput, CaptureResult, EnqueueInput, IngestInput, ModelResult, ProposalBudgets, RunJobsOptions, RunJobsReport, RuntimeJob, RuntimeOptions, RuntimeProposalRequest, RuntimeProposer, RuntimeSkill, RuntimeSource, SkillDefinition, SkillTrialInput, SkillValidation } from './types.js';
...
function errorText(error: unknown): string { return (error instanceof Error ? error.message : 'Runtime operation failed.').slice(0, 1024); }
...
export class MemoryRuntime {
  ...
  #sources(sourceIds: string[]): { sources: RuntimeSource[]; fingerprint: string; trust: MemoryTrust } {
    ids.parse(sourceIds);
    ...
    return { sources: records.map(record => ({ id: record.id, text: record.text, source: record.source, trust: record.trust })), fingerprint: hash([...seen].sort(([a], [b]) => a.localeCompare(b))), trust: records.some(record => record.trust === 'untrusted') ? 'untrusted' : 'observed' };
  }
  ...
  async ingest(input: IngestInput): Promise<CaptureResult> {
    ...
    const data = input.data === undefined ? undefined : input.data instanceof Uint8Array ? input.data.slice() : undefined;
    if (directText !== undefined ? typeof directText !== 'string' || Buffer.byteLength(directText) > maxInputBytes : !data || data.byteLength > maxInputBytes) throw new Error('Document input budget exceeded.');
    const revision = text(1024).parse(input.revision ?? createHash('sha256').update(directText ?? data!).digest('hex'));
    const ingestKey = `ingest:${hash([uri, revision])}`;
    const documentIdentity = hash(['document', uri]);
    this.#assertNotForgotten(ingestKey, documentIdentity);
    const documentVersions = () => this.#records('source', { adapter: 'document' }, true).filter(record => record.metadata.documentIdentity === documentIdentity || record.source.uri === uri);
    const extractionHead = data ? hash(documentVersions().filter(record => record.status === 'active').map(record => record.id).sort()) : undefined;
    ...
    if (directText !== undefined) {
      ...
      extracted = await boundedCall(signal => extractor({ uri, mimeType, data: data!, maxOutputBytes, signal }), input);
    }
    ...
    const record = this.memory.atomic(() => {
      ...
      const meta = { runtimeType: 'source', adapter: 'document', mimeType, extraction: data ? 'caller-supplied' : 'direct-text', ingestKey, documentIdentity };
  ...
  async #propose(job: Pick<RuntimeJob, 'kind' | 'key' | 'sourceIds' | 'fingerprint' | 'tier' | 'parentKey'>, proposer: RuntimeProposer, options: ProposalBudgets) {
    ...
    if (job.kind === 'model' && job.key) this.#retireModels(job.key, job.fingerprint);
    const response = await boundedCall(signal => proposer({ ...before.envelope, signal } as RuntimeProposalRequest), options);
    ...
    if (current.fingerprint !== before.fingerprint) throw new RuntimeEvidenceError('Source evidence changed during proposal.');
    ...
    return this.memory.atomic(() => {
      ...
      if (this.#sources(job.sourceIds).fingerprint !== before.fingerprint) throw new RuntimeEvidenceError('Source evidence changed before commit.');
      if (job.kind === 'model' && job.key) {
        ...
        this.#retireModels(job.key, job.fingerprint);
      }
      ...
      const records = parsed.map(proposal => this.memory.store({ text: proposal.text, kind: 'observation', trust: before.trust, dependencies: job.kind === 'model' ? job.sourceIds : proposal.sourceIds, source: { uri: `runtime:${job.kind}:${hash({ identity, proposal })}`, revision: before.fingerprint }, metadata: metadata({ runtimeType: job.kind === 'model' ? 'model' : 'observation', advisory: true, generation, ...(job.key ? { modelKey: job.key } : {}), tier: job.tier ?? 'overview', ...(job.parentKey ? { parentKey: job.parentKey } : {}), relevantSourceIds: job.sourceIds, fingerprint: before.fingerprint, citedSourceIds: proposal.sourceIds }), idempotencyKey: `runtime-result:${hash({ identity, proposal, ...(job.kind === 'model' ? { generation } : {}) })}` }));
  ...
  async runJobs(options: RunJobsOptions): Promise<RunJobsReport> {
    ...
    for (const candidate of this.jobs().sort((a, b) => a.jobId.localeCompare(b.jobId))) {
      ...
      try {
        ...
        report.modelCalls++; report.inputBytes += prepared.inputBytes;
        await this.#propose(job, options.proposer, { ...options, timeoutMs });
  ...
  #retireModels(key: string, fingerprint: string): void {
    this.memory.atomic(() => {
      for (const record of this.#records('model', { modelKey: key })) {
        if (record.metadata.fingerprint === fingerprint || !this.memory.isEligible(record.id)) continue;
  ...
  getModel(key: string, options: { sourceIds?: string[] } = {}): ModelResult {
    ...
    try {
      ...
      if (!this.memory.isEligible(record.id) || state.fingerprint !== record.metadata.fingerprint) throw new Error('Model source state changed.');
  ...
  createSkill(input: SkillDefinition): RuntimeSkill {
    ...
    const skillId = hash({ definition, fingerprint });
    const existing = this.#records('skill', { skillId })[0];
    if (existing) return this.getSkill(skillId)!;
    return this.#storeSkill({ id: skillId, recordId: '', state: 'candidate', definition, fingerprint, trials: [] });
  }
  ...
  #readSkill(record: MemoryRecord): RuntimeSkill {
    const payload = z.object({ id: digest, state: z.enum(['candidate', 'active', 'retired']), definition: skillSchema, fingerprint: digest, reason: text(1024).optional(), trials: z.array(validationSchema).max(32) }).strict().parse(JSON.parse(record.text));
    const lastTrial = payload.trials.at(-1);
    if (record.agentId !== this.memory.agentId || record.workspaceId !== this.memory.workspaceId || record.kind !== 'procedure' || record.visibility !== 'private' || record.metadata.skillId !== payload.id || record.metadata.skillState !== payload.state || record.metadata.advisory !== (payload.state === 'active') || record.source.uri !== `runtime:skill:${payload.id}` || record.source.revision !== payload.fingerprint || hash({ definition: payload.definition, fingerprint: payload.fingerprint }) !== payload.id || canonical(record.dependencies) !== canonical(payload.definition.evidenceIds) || !Number.isSafeInteger(record.metadata.generation) || Number(record.metadata.generation) < 1 || new Set(payload.trials.map(trial => trial.taskId)).size !== payload.trials.length || new Set(payload.trials.map(trial => trial.evidence)).size !== payload.trials.length || (payload.state === 'candidate' && (payload.trials.length || record.trust !== 'untrusted')) || (payload.state === 'active' && (record.trust === 'untrusted' || !lastTrial?.passed || !lastTrial.prerequisitesSatisfied || payload.trials.some(trial => !trial.passed || !trial.prerequisitesSatisfied) || this.memory.getOutcomeSummary(record.id).successes < 1))) throw new Error('Invalid persisted runtime skill envelope.');
    return { ...payload, recordId: record.id };
  }
  ...
  async trialSkill(input: SkillTrialInput): Promise<RuntimeSkill> {
    ...
    return this.memory.atomic(() => {
      ...
      const next = this.#storeSkill({ ...skill, state: passed ? 'active' : 'retired', trials: [...skill.trials, validation], ...(passed ? {} : { reason: validation.prerequisitesSatisfied ? 'Verified trial failed.' : 'Required prerequisites were not satisfied.' }) });
      if (passed) this.memory.recordOutcome({ memoryId: next.recordId, success: true, evidence: validation.evidence, verifier: validation.verifier, taskId: validation.taskId });
      return next;
    });
  }
