...
- `recall({ query, limit?, kinds?, includeUntrusted? })` searches active records lexically. Default limit is 10, maximum 100. Untrusted records are excluded by default. Results include scores and outcome counts; raw recall is for inspection, while `compile` adds provenance eligibility checks.
