...
`maxCandidates` defaults to 1,000 and can be set from 1 to 10,000. It bounds the ranked lexical SQL candidate set and the number of best semantic candidates retained. The semantic channel scans all eligible indexed history in the authorized scope at the requested temporal cutoff; it does not restrict search to a recent-record window. A scan deadline or cancellation fails the operation instead of returning a partial scan. Hybrid fusion combines up to 100 lexical turns with up to `maxCandidates` retained semantic turns before selecting K. The effective budget appears in `report.limits.maxCandidates`; it limits retained candidates, not total scan work. This v1 runner explicitly uses the earlier overlap scorer for its lexical and hybrid lexical channels, independently of the product's BM25 default.
